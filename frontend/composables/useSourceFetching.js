...
export function useSourceFetching({ jurisdiction, fetchOupChapter = false, fetchPrimarySource = false }) {
    ...
    async function fetchPrimarySourceData() {
        ...
        try {
            ...
            error.value = err instanceof Error ? err.message : 'Failed to fetch primary source'
        } finally {
            loading.value = false
        }
    }
    ...
    async function fetchOupChapterSourceData() {
        ...
        try {
            ...
            error.value = err instanceof Error ? err.message : 'Failed to fetch OUP JD Chapter source'
        } finally {
            loading.value = false
        }
    }
