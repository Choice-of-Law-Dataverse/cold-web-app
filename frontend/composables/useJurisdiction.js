...
export function useJurisdiction() {
  ...
  async function fetchSpecialists(jurisdictionName) {
    ...
    try {
      ...
      // If main jurisdiction data lacks Specialists or it's empty, fill it
      if (jurisdictionData.value) {
        const current = jurisdictionData.value.Specialists
        const isEmpty =
          current == null ||
  ...
  async function fetchJurisdiction(iso3) {
    ...
    try {
      ...
      // Extract the required values (preserve Specialists from API if present)
      jurisdictionData.value = {
        ...
        Specialists: data?.Specialists ?? specialists.value ?? '',
      }
