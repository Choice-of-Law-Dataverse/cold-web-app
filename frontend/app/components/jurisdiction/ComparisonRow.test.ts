...
import { ComparisonStateKey, type Row } from "./comparisonState";
import { computed, ref } from "vue";

const USkeletonStub = { template: '<div class="u-skeleton-mock"></div>' };
const UIconStub = {
  template: '<span class="u-icon-mock"></span>',
  props: ["name", "title"],
};
const UTooltipStub = {
  template: '<div class="u-tooltip-mock"><slot /></div>',
  props: ["text", "disabled"],
};
...
describe("ComparisonRow", () => {
  ...
  const defaultState = {
    ...
    getAnswerLink: (coldId: string, questionId: string) =>
  ...
  const createWrapper = (rowOverride = {}, stateOverride = {}) => {
    return mount(ComparisonRow, {
      props: {
        row: { ...defaultRow, ...rowOverride } as Row,
      },
      global: {
        components: {
  ...
  it("renders bold questions", () => {
    const wrapper = createWrapper(
      {},
      { isBoldQuestion: (id: string) => id === "01-P" },
    );
    const qCell = wrapper.find(".comparison-cell--question");
  ...
  it("displays a dash for 'Not applicable'", () => {
    const wrapper = createWrapper({
      answers: { US: "Yes", UK: "Not applicable" },
    });
    const aCells = wrapper.findAll(".comparison-cell--answer");
