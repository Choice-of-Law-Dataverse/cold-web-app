...
export function getSortedProvisionIds(
  ...
  return [...ids].sort((a, b) => {
    const raValue = rankingMap[a];
    const rbValue = rankingMap[b];
    const ra = raValue ?? Number.POSITIVE_INFINITY;
    const rb = rbValue ?? Number.POSITIVE_INFINITY;

    if (!Number.isFinite(ra) && !Number.isFinite(rb)) return 0;
    if (!Number.isFinite(ra)) return 1; // unranked go last
